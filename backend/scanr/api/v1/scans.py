...
def _validate_profile_json(raw: str) -> str:
    """Validate profile_json for the scans API, translating rejects into 400s.

    The schema itself lives in scanr.schemas.profile so that every writer of
    profile_json (scans, schedules, templates) shares one validator — see the
    module docstring there for why an unvalidated port_range is dangerous.
    """
    try:
        return validate_profile_json(raw)
