    ...
    async def _probe(self, ip: str, port: int) -> FindingData | None:
        ...
        try:
            async with httpx.AsyncClient(verify=False, timeout=6.0) as client:
                ...
                try:
                    ...
                    if r.status_code == 200 and (
                        ...
                        evidence_parts.append(
                            f"GET /v2/keys → {r.status_code} ({len(r.text)} bytes)"
                        )
                except Exception:
                    pass

                # Check v3 members endpoint
                try:
                ...
                # Check health endpoint (confirms it's etcd)
                is_etcd = False
                try:
                    r = await client.get(f"{base}/health")
                    if r.status_code == 200 and (
                        "health" in r.text or "etcd" in r.text.lower()
                    ):
                        is_etcd = True
                ...
                if not evidence_parts and not is_etcd:
                    return None
                if not evidence_parts:
                    return None  # Only health confirmed but no data exposure

                # Try to check for Kubernetes secrets path
                try:
                ...
                return FindingData(
                    ...
                    description=(
                        f"The etcd cluster at {ip}:{port} is accessible without authentication. "
            ...
            logger.debug("etcd probe failed %s:%d: %s", ip, port, exc)
        return None
